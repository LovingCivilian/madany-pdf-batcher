...
from core.constants import DEFAULT_TIMESTAMP_CONFIG, ALL_PAPER_KEYS
from core.anchor import compute_anchor_for_pdf

class TimestampConfigurationDialog(BaseConfigurationDialog):
    """Timestamp Configuration Dialog inheriting from Base."""

    def __init__(self, parent: Optional[QWidget] = None, available_fonts: Optional[Dict] = None):
        super().__init__(parent, title="Timestamp Configuration")
        self.available_fonts = available_fonts or {}
        
        default_cfg = DEFAULT_TIMESTAMP_CONFIG.copy()
    ...
    def _init_font_section(self, layout: QVBoxLayout):
        ...
        for chk in (self.bold_checkbox, self.italic_checkbox, self.underline_checkbox, self.strike_checkbox):
    ...
    def _init_position_section(self, layout: QVBoxLayout):
        ...
        # Default for Timestamp is usually Bottom Right
        for btn in self.position_btn_group.buttons():
            if btn.text() == "Bottom Right":
                btn.setChecked(True)
                break
        layout.addWidget(group)
    ...
    def _load_config_into_ui_no_preview(self, key: Tuple[str, str]) -> None:
        ...
        target_pos = cfg.get("position", "Bottom Right")
        for btn in self.position_btn_group.buttons():
    ...
    def _get_current_ui_config(self) -> Dict:
        ...
        return {
            ...
            "line_gap": 0, # Force 0 for timestamp
            "position": pos_text,
    ...
    def _draw_preview_content(self, page: fitz.Page, page_w: float, page_h: float, config: Dict) -> None:
        """Draw timestamp content onto the preview page."""
        sample_text = "2023-11-30 14:30"
